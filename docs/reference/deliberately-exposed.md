...
| `/etc/claude-sandbox.conf` | r | Host-global sandbox config (`workspace-root`, `no-forge`, `allow-write`, `pass-env`, `egress-jail`, `allow-ip`), seeded by `install.sh` from the installing clone's `.devcontainer/claude-sandbox.conf` and read by the shadow at launch. Lives at `/etc`, **not** in the rw-bound workspace, so a compromised session can't rewrite it to widen the next launch's binds. Edit `/etc/claude-sandbox.conf` directly (unsandboxed root shell) to change it; edits are per-devcontainer and reset by a rebuild/re-install |
